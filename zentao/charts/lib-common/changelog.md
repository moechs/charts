...
# 1.1.23 2025.04.11 by qisy
- lib-common.image.autotag render zentao image with phpVersion

# 1.1.22 2025.01.09 by qisy
- 不再将 _custom 的值合并到 custom，改用 $custom 依次获取
 
# 1.1.21 2024.10.25 by qisy
- workflow support hostNetwork
- env support fieldRef

# 1.1.20 2024.10.24 by qisy
- configmaps 和 secrets 可以通过键 _ 来生成当前组件名称的资源
- envFrom 支持 _ 
- envFrom 支持 map 结构的元素，可以跨组件相互引用
- 修复 statefulset 副本数管理问题

# 1.1.19 2024.7.17 by qisy
- fix empty service.externalIPs

# 1.1.18 2024.7.9 by qisy
- service externalIPs 支持引用

# 1.1.17 2024.6.12 by qisy
- DaemonSet 支持 hostNetwork
- service 支持 headless 类型
- service 支持 externalIPs

# 1.1.16 2024.5.1701 by qisy
- configmap 可被设置为自由修改模式，不跟随 helm 渲染

# 1.1.15 2024.4.15 by qisy
- 支持 Statefulset 以及动态数据卷功能
- 修正了 app.component.fullname 生成依赖 Chart下的子组件名称
