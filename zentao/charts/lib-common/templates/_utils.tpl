...
{{- else -}}
  {{- if (kindIs "string" .value) }}
    {{- if hasPrefix "$ref" .value }}
      {{- $path := trimPrefix "$ref:" .value -}}
      {{ include "lib-common.utils.getValueByPath" (dict "path" $path "ctx" .ctx) }}
    {{- else if (hasPrefix "$custom:" .value) }}
      {{- $key := substr 8 (len .value) .value -}}
      {{- include "lib-common.utils.readCustom" (dict "key" $key "ctx" .ctx) }}
...
{{/*
Read value from custom map, or _custom array
Usage {{ include "lib-common.utils.readCustom" (dict "key" "value" "ctx" $) }}
*/}}
{{- define "lib-common.utils.readCustom" -}}
{{- if hasKey .ctx.Values.custom .key -}}
  {{- get .ctx.Values.custom .key }}
{{- else }}
  {{- $found := false }}
  {{- range .ctx.Values._custom }}
    {{- if eq .name $.key }}
      {{- default "" .default }}
      {{- $found = true }}
    {{- end }}
...
{{/*
Usage: {{ include "lib-common.utils.stringToList" (dict "value" .value "separator" "" "ctx" .ctx ) }}
convert string to a list, or return origin list
value should be list or string
*/}}
{{- define "lib-common.utils.stringToList" -}}
{{- if (kindIs "string" .value ) }}
{{- regexSplit .separator .value -1 | toYaml }}
{{- else }}
{{- toYaml .value }}
{{- end }}
{{- end -}}